            ...
            try:
                ...
                if price <= 0:
                    ...
                    with open("balance.txt", "w") as file:
                        ...
                        balance_label.config(text=f"Balance: {balance-totalcost}               ")
...
def delete_item():
    ...
    total = float(values[4])  # total cost of that purchase

    with open("balance.txt", "r") as f:
        balance = float(f.read())

    balance += total  # add back the money
...
# Quantity
    tk.Label(edit_win, text="Quantity").grid(row=0, column=0, padx=5, pady=5)
    qty_entry = tk.Entry(edit_win)
    qty_entry.grid(row=0, column=1, padx=5, pady=5)
    qty_entry.insert(0, values[0])
    entries.append(qty_entry)

# Unit (DROPDOWN)
    tk.Label(edit_win, text="Unit").grid(row=1, column=0, padx=5, pady=5)
    unit_var = tk.StringVar(value=values[1])
    unit_menu = tk.OptionMenu(edit_win, unit_var, *unit)
...
# Price
    tk.Label(edit_win, text="Price").grid(row=3, column=0, padx=5, pady=5)
...
# Category (DROPDOWN)
    tk.Label(edit_win, text="Category").grid(row=4, column=0, padx=5, pady=5)
    category_var = tk.StringVar(value=values[5])
    category_menu = tk.OptionMenu(edit_win, category_var, *category)
    ...
    def save_edit():
        ...
        unit = unit_var.get()
        desc_val = desc_entry.get().replace(" ", "-")
        ...
        balance = balance + old_total - new_total
        ...
        table.item(item, values=(quantity, unit, desc_val, price, new_total, category, date))
